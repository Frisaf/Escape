...
def intro():
    ...
    wprint(f"{RED}UPPLÄSARE: {RESET}{YELLOW}Fantastiskt! Välkommen, {GREEN}{user_name}{YELLOW}! Innan vi startar spelet så finns det några saker som du behöver veta.\nFörst och främst så finns det två typer av kommandon: {GREEN}använd{YELLOW} och {GREEN}gå{YELLOW}. Kommandot använd gör så att du interagerar med olika saker i rummet, så om du till exempel vill interagera med äpplet så skulle du skriva {GREEN}'använd äpple'{YELLOW} eller {GREEN}'a äpple'{YELLOW} i terminalen och om du vill flytta dig till höger så skriver du {GREEN}'gå höger'{YELLOW} eller {GREEN}'g h'{YELLOW } i terminalen (andra aliaskombinationer fungerar också).{YELLOW} Om du går bakåt kommer du alltid att gå tillbaka till det föregående rummet om ingenting annat sägs. Tillgängliga kommandon: {GREEN}höger/h, vänster/v, framåt/f, bakåt/b\n{YELLOW}Föremål som du kan interagera med är skrivna med {CYAN}turkos{YELLOW} text.\nMed låt oss nu start spelet!{RESET}")
    wprint("")
